...
export const TeacherDashboard: React.FC<TeacherDashboardProps> = ({
  ...
  // NEW: Single Reminder Handlers
  const handleSingleFeeReminder = (student: Student) => {
      ...
      onUpdateStudent({ ...student, feeReminder: { month: currentDebt, dateSet: new Date().toISOString() } });
      onShowNotification(`تم إرسال تذكير للطالب ${student.name}`, 'success');
  };

  const handleCancelFeeReminder = (student: Student) => {
      onUpdateStudent({ ...student, feeReminder: undefined });
      onShowNotification(`تم إلغاء التذكير للطالب ${student.name}`, 'success');
  };
                        ...
                        <div className="bg-paper dark:bg-gray-800 p-4 rounded-3xl shadow-lg border border-white dark:border-gray-700 animate-slide-up min-h-[300px] mb-8">
                            {activeTab === 'FEES' && (
                                <div className="animate-fade-in">
                                    ...
                                    <div className="mb-4">
                                        <div className="flex justify-between items-center mb-2"><h4 className="text-sm font-bold text-darkBrown dark:text-white">سجل: <span className="text-secondaryDark dark:text-yellow-500">{activeFeePage || 'لم يتم الاختيار'}</span></h4>{activeFeePage && (<button onClick={handleBulkFeeReminder} className="bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm hover:bg-red-100 dark:hover:bg-red-900/50">🔔 تذكير للكل</button>)}</div>
                                        <input 
                                            type="text" 
                                    ...
                                    {activeFeePage ? (
                                        <div className="space-y-3">
                                            ...
                                            feeFilteredStudents.map(s => { 
                                                ...
                                                return (
                                                <div key={s.id} className={`bg-white dark:bg-gray-700 p-3 rounded-xl border shadow-sm relative ${hasPaidThisPage ? 'border-green-300 bg-green-50 dark:bg-green-900/20' : 'border-gray-200 dark:border-gray-600'}`}>
                                                    <div className="flex justify-between items-center mb-2">
                                                        <span className="font-bold text-sm text-darkBrown dark:text-white">{s.name}</span>
                                                        <div className="flex gap-1">
                                                            {s.feeReminder && !hasPaidThisPage && <span className="bg-red-100 text-red-600 text-[10px] font-bold px-2 py-0.5 rounded-full animate-pulse">🔔 منبه</span>}
                                                    ...
                                                    {!hasPaidThisPage && (
                                                        <>
                                                            <div className="flex gap-2 items-center mb-2">
                                                                <input type="number" placeholder="المبلغ" className="w-20 p-2 border rounded-lg text-xs text-center font-bold bg-gray-50 dark:bg-gray-600 dark:text-white" value={entry.amount} onChange={(e) => updatePaymentEntry(s.id, 'amount', e.target.value)} />
                                                                <input type="text" placeholder="ملاحظات" className="flex-1 p-2 border rounded-lg text-xs dark:bg-gray-600 dark:text-white" value={entry.notes} onChange={(e) => updatePaymentEntry(s.id, 'notes', e.target.value)} />
                                                            </div>
                                                            <div className="flex gap-2">
                                                                <button onClick={() => handleConfirmPaymentInline(s)} className="flex-1 bg-green-600 text-white p-2 rounded-lg text-xs font-bold shadow hover:bg-green-700">تأكيد دفع 💰</button>
                                                                <button onClick={() => handleSingleFeeReminder(s)} className="bg-red-50 text-red-600 border border-red-200 p-2 rounded-lg text-[10px] font-bold hover:bg-red-100" title="تذكير ولي الأمر">إرسال تذكير 🔔</button>
                                                                <button onClick={() => handleCancelFeeReminder(s)} className="bg-gray-50 text-gray-500 border border-gray-200 p-2 rounded-lg text-[10px] font-bold hover:bg-gray-100" title="إلغاء التذكير">إلغاء التذكير ❌</button>
                                                            </div>
                                                        </>
                                                    )}
                                                </div>); 
                            ...
                            {/* Rest of Tabs: ADAB, ANNOUNCEMENTS, STATS, DELETE - preserving old logic) */}
                            {activeTab === 'ADAB' && (
                                <div>
