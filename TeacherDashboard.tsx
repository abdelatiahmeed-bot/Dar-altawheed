    ...
    return (
        <div className={`p-4 rounded-xl border-2 ${colorClass} mb-3 relative animate-fade-in`}>
            <div className="flex justify-between items-center mb-3">
                <h4 className="font-bold text-[#3d2e18]">{title}</h4>
                {canRemove && (
            ...
            <div className="grid grid-cols-4 gap-1 mb-3">
                {['SURAH', 'RANGE', 'JUZ', 'MULTI'].map(type => (
                    <button
                        key={type}
                        className={`py-1 px-1 rounded-lg text-[10px] font-bold transition whitespace-nowrap ${data.type === type ? 'bg-[#5c6b48] text-white' : 'bg-white text-[#5c6b48] border border-[#5c6b48]'}`}
            ...
            <div className="space-y-3">
                {data.type === 'JUZ' ? (
                    <select
                        className="w-full p-2 border border-[#e6e2d3] rounded-lg bg-white text-[#3d2e18]"
                    ...
                    <div className="bg-white p-2 rounded-lg border border-[#e6e2d3]">
                        ...
                        <button onClick={handleAddMultiSurah} className="w-full py-1 text-xs border border-dashed border-[#a89060] text-[#a89060] rounded hover:bg-[#f4f1ea]">+ إضافة سورة أخرى</button>
                    </div>
                ) : (
                    <>
                        <div className="flex gap-2">
                            <div className="flex-1">
                                <label className="text-[10px] font-bold text-[#8c7b60] block mb-1">من سورة</label>
                                <select
                                    className="w-full p-2 border border-[#e6e2d3] rounded-lg bg-white text-sm"
                                    value={data.name}
                            ...
                            {isRange && (
                                <div className="flex-1">
                                    <label className="text-[10px] font-bold text-[#8c7b60] block mb-1">إلى سورة</label>
                                    <select
                                        className="w-full p-2 border border-[#e6e2d3] rounded-lg bg-white text-sm"
                        ...
                        {isSurah && (
                            <div className="flex items-center gap-2 bg-white p-2 rounded-lg border border-[#e6e2d3]">
                                <div className="flex-1">
                                    <label className="text-[10px] text-[#8c7b60] block mb-0.5">من آية</label>
                                    <select
                                ...
                                <span className="text-[#a89060] mt-4">إلى</span>
                                <div className="flex-1">
                                    <label className="text-[10px] text-[#8c7b60] block mb-0.5">إلى آية</label>
                                    <select
                                        className="w-full p-1 border rounded text-center font-bold bg-gray-50"
                                        value={data.ayahTo}
                ...
                {!hideGrade && !isMulti && (
                    <div>
                        <label className="text-[10px] font-bold text-[#8c7b60] block mb-1">التقييم</label>
                        <select
                            className="w-full p-2 border border-[#e6e2d3] rounded-lg bg-white text-sm font-bold focus:ring-2 focus:ring-[#5c6b48] outline-none"
                            value={data.grade}
...
// --- TAB BUTTON COMPONENT (UPDATED STYLE) ---
const TabButton = ({ id, label, icon, isActive, onClick, compact = false }: { id: string, label: string, icon?: string, isActive: boolean, onClick: () => void, compact?: boolean }) => (
    <button
        onClick={onClick}
        className={`flex items-center gap-2 px-3 py-2 rounded-xl whitespace-nowrap transition-all duration-200 shadow-sm border ${isActive
                ? 'bg-[#5c6b48] text-white border-[#5c6b48] font-bold shadow-md'
                : 'bg-white text-[#3d2e18] border-[#e6e2d3] hover:bg-[#f4f1ea]'
...
export const TeacherDashboard: React.FC<TeacherDashboardProps> = ({
    ...
    // --- UPDATED CARD DESIGN (MATCHING SCREENSHOT) ---
    const renderStudentCard = (student: Student) => {
        ...
        // UPDATED: Islamic Card Style
        let cardBg = "bg-[#fdfcf8]";
        let statusIcon = (<span className="text-gray-400 text-xl font-bold">➜</span>);

        if (hasDraft) {
            statusIcon = (<span className="text-amber-600 text-xs font-bold px-2 py-1 bg-amber-100 rounded-full">مسودة</span>);
        } else if (hasLogToday) {
            if (isAbsentToday) {
                statusIcon = (<span className="text-red-600 text-xs font-bold px-2 py-1 bg-red-100 rounded-full">غائب</span>);
            } else {
                statusIcon = (<span className="text-emerald-600 text-xs font-bold px-2 py-1 bg-emerald-100 rounded-full">تم</span>);
            }
        }

        return (
            <div key={student.id} onClick={() => handleOpenStudent(student)} className={`relative flex items-center justify-between p-4 mb-3 rounded-2xl shadow-sm border border-[#e6e2d3] cursor-pointer transition-transform active:scale-[0.99] ${cardBg} overflow-hidden`}>
                {/* Right Side Indicator (Vertical Bar) */}
                <div className="absolute right-0 top-4 bottom-4 w-1.5 bg-[#a89060] rounded-l-full"></div>

                <div className="flex items-center gap-4 w-full pr-4">
                    {/* Initial Icon */}
                    <div className="w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold border-2 border-[#e6e2d3] text-[#a89060] bg-[#f4f1ea]">
                        |
                    </div>

                    <div className="flex-1 text-right">
                        <h3 className="font-bold text-[#3d2e18] text-lg">{student.name}</h3>
                        <p className="text-sm text-[#8c7b60] font-mono mt-1">كود: {student.parentCode}</p>
                    </div>
                    {/* Left Arrow Icon */}
                    <div className="w-10 h-10 flex items-center justify-center text-[#a89060]">
                        {statusIcon}
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="min-h-screen bg-[#f4f1ea] font-cairo pb-24 relative overflow-hidden">
            ...
            <div className="bg-white/80 backdrop-blur shadow-sm sticky top-0 z-50 border-b border-[#e6e2d3]">
                <div className="px-4 py-3 flex justify-between items-center">
                    {selectedStudentId ? (
                        <div className="flex items-center gap-3 w-full animate-slide-right">
                            <button onClick={handleCloseStudent} className="p-2 bg-[#f4f1ea] rounded-full hover:bg-[#e6e2d3] transition text-[#5c6b48] shadow-sm">⬅</button>
                            <div className="flex-1">
                                <h1 className="text-xl font-bold text-[#3d2e18] truncate">{selectedStudent?.name}</h1>
                                <div className="flex items-center gap-2 mt-1">
                                    {isEditingPhone ? (
                                        ...
                                        <p className="text-sm text-[#8c7b60] font-bold flex items-center gap-2">
                                            <a href={`tel:${selectedStudent?.parentPhone}`} className="hover:text-[#5c6b48] transition-colors">📞 {selectedStudent?.parentPhone || 'لا يوجد هاتف'}</a>
                                            <button onClick={() => { setIsEditingPhone(true); setNewPhoneVal(selectedStudent?.parentPhone || ''); }} className="text-[#5c6b48] bg-[#f4f1ea] p-1.5 rounded-lg text-lg">✎</button>
                                        </p>
                                    )}
                                </div>
                            </div>
                        ...
                        <>
                            ...
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 bg-[#5c6b48] rounded-full flex items-center justify-center text-white text-xl shadow-sm border-2 border-[#a89060]">👳‍♂️</div>
                                <div>
                                    <h1 className="font-bold text-[#3d2e18] text-lg">أهلاً بك</h1>
                                    <p className="text-xs text-[#a89060] font-bold">{teacherName}</p>
                                </div>
                            </div>
                            {/* Logout Button */}
                            <div className="flex gap-2">
                                <button onClick={onLogout} className="text-white bg-[#a89060] hover:bg-[#8f7a50] px-3 py-1.5 rounded-lg border border-[#a89060] transition shadow-sm">
                ...
                {/* TABS SCROLL - ORIGINAL TABS WITH NEW STYLE */}
                {!selectedStudentId && (
                    <div className="px-2 pb-2">
                        <div className="flex overflow-x-auto gap-2 py-1 touch-pan-x no-scrollbar" style={{ scrollbarWidth: 'none' }}>
                            <TabButton id="LIST" icon="📋" label="الطلاب" isActive={activeTab === 'LIST'} onClick={() => setActiveTab('LIST')} />
            ...
            <div className="p-4 max-w-lg mx-auto relative z-10">
                {!selectedStudentId ? (
                    <>
                        {activeTab === 'LIST' && (
                            <div className="space-y-3 animate-slide-up">
                                <div className="bg-[#f4f1ea] px-4 py-3 rounded-xl border border-[#e6e2d3] flex flex-col gap-2 text-[#5c6b48] font-bold shadow-sm mb-2">
                                    <div className="flex justify-between items-center">
                                        <span>عدد الطلاب في المجموعة:</span>
                                        <span className="bg-white px-3 py-1 rounded-full text-[#5c6b48] shadow-sm">{students.length}</span>
                                    </div>
                                    {students.length > 0 && (
                                        <div className="flex gap-2 mt-2 border-t border-[#e6e2d3] pt-2">
                                            <button onClick={() => setSortMethod('ALPHABETICAL')} className={`flex-1 text-xs py-1.5 rounded-lg transition ${sortMethod === 'ALPHABETICAL' ? 'bg-[#5c6b48] text-white' : 'bg-white text-[#5c6b48] hover:bg-[#e6e2d3]'}`}>أبجدياً (أ-ي)</button>
                                            <button onClick={() => setSortMethod('CODE')} className={`flex-1 text-xs py-1.5 rounded-lg transition ${sortMethod === 'CODE' ? 'bg-[#5c6b48] text-white' : 'bg-white text-[#5c6b48] hover:bg-[#e6e2d3]'}`}>بالكود (الأرقام)</button>
                                        </div>
                                    )}
                                </div>
                                {sortedStudents.length === 0 ? (
                                    <div className="text-center py-10 text-gray-400 bg-white rounded-xl border border-dashed border-[#e6e2d3]">
                                        <p className="text-lg">لا يوجد طلاب مسجلين.</p>
                                        <p className="text-xs mt-2 text-[#5c6b48] font-bold cursor-pointer" onClick={() => setActiveTab('ADD')}>+ أضف طلابك الآن</p>
                                    </div>
                                ) : (
                        ...
                        {activeTab === 'ADD' && (
                            <div className="bg-white p-6 rounded-xl shadow-lg border border-[#e6e2d3] animate-slide-up">
                                <div className="text-center mb-6">
                                    <div className="w-16 h-16 bg-[#f4f1ea] rounded-full flex items-center justify-center text-3xl mx-auto mb-2 border-4 border-[#e6e2d3]">👤</div>
                                    <h3 className="font-bold text-[#3d2e18] text-lg">إضافة طالب جديد</h3>
                                    <p className="text-[#8c7b60] text-sm">أضف الطالب للحلقة ليتمكن ولي الأمر من المتابعة</p>
                                </div>
                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-bold text-[#3d2e18] mb-1">اسم الطالب</label>
                                        <input className="w-full p-3 border rounded-lg bg-[#fdfcf8] focus:bg-white transition" placeholder="الاسم الثلاثي" value={newStudentName} onChange={e => setNewStudentName(e.target.value)} />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-[#3d2e18] mb-1">كود الطالب (لولي الأمر)</label>
                                        <input className="w-full p-3 border rounded-lg bg-[#fdfcf8] focus:bg-white transition font-mono text-center tracking-widest text-lg" placeholder="مثال: 101" value={newStudentCode} onChange={e => setNewStudentCode(e.target.value)} />
                                    </div>
                                    <Button onClick={() => {
                                    ...
                                    }} className="w-full py-3 shadow-md text-lg bg-[#5c6b48] hover:bg-[#4a563a]">تسجيل الطالب +</Button>
                                </div>
                            </div>
                        )}
                        ...
                        {activeTab === 'ADAB' && (
                            <div className="animate-slide-up space-y-4">
                                <div className="bg-white p-5 rounded-xl border border-[#e6e2d3] shadow-sm relative overflow-hidden">
                                    {editingAdabId && (
                                        <div className="absolute top-0 right-0 bg-[#a89060] text-white px-3 py-1 rounded-bl-lg font-bold text-xs z-20">
                                            وضع التعديل ✏️
                                        </div>
                                    )}
                                    <div className="text-center mb-6 relative z-10">
                                        <span className="text-4xl block mb-2">🌟</span>
                                        <h3 className="font-bold text-[#3d2e18] text-xl">{editingAdabId ? "تعديل درس الآداب" : "مجلس الآداب والقيم"}</h3>
                                    </div>
                                    <div className="space-y-4 relative z-10">
                                        <div>
                                            <label className="block text-xs font-bold text-[#3d2e18] mb-1">عنوان الدرس</label>
                                            <input className="w-full p-3 border rounded-lg bg-[#fdfcf8] shadow-sm focus:ring-2 focus:ring-[#5c6b48] outline-none transition" value={adabTitle} onChange={e => setAdabTitle(e.target.value)} placeholder="مثال: بر الوالدين" />
                                        </div>
                                        <div className="bg-[#f4f1ea] p-4 rounded-xl shadow-sm border border-[#e6e2d3]">
                                            <h4 className="font-bold text-[#5c6b48] text-sm mb-3 flex items-center gap-2">إضافة سؤال تفاعلي</h4>
                                            <input className="w-full p-2 border rounded mb-2 text-sm bg-white focus:bg-white" placeholder="نص السؤال..." value={currentQuestion} onChange={e => setCurrentQuestion(e.target.value)} />
                                            ...
                                            <Button onClick={handleAddToQuestionList} variant="secondary" className="w-full text-xs mt-3 py-2 bg-[#a89060] hover:bg-[#8f7a50] text-white">إضافة السؤال للقائمة ⬇️</Button>
                                        </div>
                                    </div>
                                    {editingAdabId && (
                                ...
                                {adabQuestionsList.length > 0 && (
                                    <div className="bg-white p-4 rounded-xl shadow-md border border-[#e6e2d3] animate-slide-up">
                                        <h4 className="font-bold text-[#3d2e18] text-sm mb-3 border-b pb-2">الأسئلة المضافة ({adabQuestionsList.length})</h4>
                                        <ul className="space-y-2 mb-4">
                                            {adabQuestionsList.map((q, i) => (
                                                <li key={q.id} className="text-xs bg-[#fdfcf8] p-3 rounded-lg border border-[#e6e2d3] flex justify-between items-start">
                                                    <div><span className="font-bold text-[#5c6b48] block mb-1">س {i + 1}: {q.question}</span><span className="text-gray-500">ج: {q.correctAnswer}</span></div>
                                        ...
                                        <Button onClick={handlePublishAdabLesson} className="w-full bg-[#5c6b48] hover:bg-[#4a563a] text-white shadow-lg py-3 text-lg">{editingAdabId ? "💾 حفظ التعديلات" : "🚀 نشر الدرس الآن"}</Button>
                                    </div>
                                )}
                                ...
                                {adabArchive.length > 0 && (
                                    <div className="mt-6">
                                        <h4 className="font-bold text-[#5c6b48] mb-3 border-b pb-2 flex justify-between items-center">
                                            <span>🗄️ أرشيف الدروس السابقة</span>
                                        </h4>
                                        <div className="space-y-3">
                                            {adabArchive.map(session => (
                                                <div key={session.id} className="bg-white p-3 rounded-xl border border-[#e6e2d3] flex justify-between items-center shadow-sm">
                                                    <div>
                                                        <p className="font-bold text-[#3d2e18] text-sm">{session.title}</p>
                                                        <p className="text-[10px] text-[#8c7b60]">{formatSimpleDate(session.date)} • {session.quizzes.length} أسئلة</p>
                                                    </div>
                                                    <div className="flex gap-2">
                                                        <button onClick={() => handleEditAdabSession(session)} className="bg-[#f4f1ea] text-[#5c6b48] px-2 py-1 rounded text-xs font-bold border border-[#e6e2d3]">تعديل ✎</button>
                        ...
                        {activeTab === 'ATTENDANCE' && (
                            <div className="animate-slide-up bg-white p-5 rounded-xl shadow-lg border border-[#e6e2d3]">
                                <div className="text-center mb-4">
                                    <h3 className="font-bold text-[#a89060] text-lg">سجل الحضور السريع</h3>
                                    <p className="text-gray-500 text-sm">اضغط على الطالب لتغيير حالته (غياب/عذر/حضور)</p>
                                </div>

                                {unloggedStudents.length === 0 ? (
                                    <div className="text-center py-8 bg-[#f4f1ea] rounded-xl border border-[#e6e2d3]">
                                        <p className="text-[#5c6b48] font-bold">🎉 تم تسجيل جميع الطلاب اليوم!</p>
                                    </div>
                                ) : (
                                    <>
                                        <button
                                            onClick={handleSelectAllAbsent}
                                            className="w-full mb-4 bg-[#f4f1ea] text-[#5c6b48] px-4 py-2 rounded-lg font-bold text-xs border border-[#e6e2d3] hover:bg-[#e6e2d3] transition"
                                        >
                                            ❌ تحديد الكل كغياب
                                        </button>
                                        <div className="grid grid-cols-2 gap-3 max-h-96 overflow-y-auto p-1 mb-4">
                                            {unloggedStudents.map(s => {
                                                ...
                                                let cardClass = "bg-white border-[#e6e2d3] text-gray-700";
                                                let statusIcon = "⬜";
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all duration-200 select-none ${cardClass}`}
                                                    >
                                        ...
                                        <div className="flex justify-between items-center bg-[#f4f1ea] p-3 rounded-lg border border-[#e6e2d3] text-xs text-gray-500 mb-4">
                        ...
                        {activeTab === 'STATS' && (
                            <div className="animate-slide-up bg-white p-5 rounded-xl shadow-sm border border-[#e6e2d3]">
                                <h3 className="font-bold text-[#3d2e18] text-lg mb-4 flex items-center gap-2">📊 إحصائيات الحضور اليومية</h3>
                                <div className="mb-6">
                                    <label className="block text-sm font-bold text-gray-600 mb-1">اختر التاريخ</label>
                                    <input type="date" className="w-full p-2 border rounded-lg bg-[#f4f1ea]" value={statsDate} onChange={(e) => setStatsDate(e.target.value)} />
                                </div>
                                <div className="grid grid-cols-2 gap-4 mb-6">
                                    <div className="bg-[#f4f1ea] p-4 rounded-xl text-center border border-[#e6e2d3] shadow-sm relative overflow-hidden">
                                        <p className="text-[#5c6b48] font-bold text-sm z-10 relative">✅ الحضور</p>
                                        <p className="text-4xl font-black text-[#5c6b48] mt-2 z-10 relative">{statsData.present.length}</p>
                                    </div>
                                ...
                                <div className="space-y-6">
                                    <div className="bg-[#f4f1ea] rounded-xl p-3 border border-[#e6e2d3]">
                                        <h4 className="font-bold text-[#5c6b48] text-sm mb-3 flex items-center gap-1 border-b pb-2"><span>🟢</span> قائمة الحضور ({statsData.present.length})</h4>
                                        {statsData.present.length > 0 ? (
                                            <div className="grid grid-cols-2 gap-2">
                                                {statsData.present.map(s => (<div key={s.id} className="bg-white px-2 py-1.5 rounded border border-[#e6e2d3] text-xs font-bold text-gray-700 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-500"></span>{s.name}</div>))}
                                            </div>
                                    ...
                                    <div className="bg-[#f4f1ea] rounded-xl p-3 border border-[#e6e2d3]">
                                        ...
                                        {statsData.absent.length > 0 ? (
                                            <div className="grid grid-cols-2 gap-2">
                                                {statsData.absent.map(s => (<div key={s.id} className="bg-white px-2 py-1.5 rounded border border-[#e6e2d3] text-xs font-bold text-gray-700 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-red-500"></span>{s.name}</div>))}
                                            </div>
                        ...
                        {activeTab === 'ANNOUNCEMENTS' && (
                            <div className="animate-slide-up space-y-4">
                                <div className="bg-white p-5 rounded-xl border border-[#e6e2d3] shadow-sm">
                                    <h3 className="font-bold text-[#3d2e18] text-lg mb-4 text-center">📢 لوحة الإعلانات</h3>
                                    <div className="mb-4">
                                        <label className="block text-xs font-bold text-gray-600 mb-1">نوع الإعلان</label>
                                        <select className="w-full p-2 border rounded-lg bg-[#f4f1ea] mb-3" value={announcementType} onChange={(e) => setAnnouncementType(e.target.value as 'GENERAL' | 'EXAM')}>
                                            <option value="GENERAL">إعلان عام</option>
                                            <option value="EXAM">اختبار شهر</option>
                                        </select>

                                        {announcementType === 'EXAM' ? (
                                            <div className="space-y-3 bg-[#f4f1ea] p-3 rounded-lg border border-[#e6e2d3] mb-3">
                                                <div>
                                                    <label className="block text-xs font-bold text-gray-600 mb-1">اختر المحفظ المختبر</label>
                                                    <select
                                                        className="w-full p-4 text-xl border-2 border-[#a89060] rounded-xl bg-white text-gray-900 font-bold outline-none cursor-pointer"
                                                        value={examTesterId}
                                                    ...
                                                    {examDays.length > 0 && (
                                                        <div className="space-y-1">
                                                            {examDays.map((day, idx) => (
                                                                <div key={idx} className="flex justify-between items-center bg-white p-2 rounded border border-[#e6e2d3] text-xs">
                                                                    <span className="font-bold text-[#3d2e18]">📅 {formatDateWithDay(day.date)}: <span className="text-gray-600 font-normal">{day.description === 'حفظ' ? 'حفظ جديد' : day.description}</span></span>
                                            ...
                                            <textarea className="w-full p-3 border rounded-lg text-sm bg-[#f4f1ea] focus:bg-white transition mb-2" placeholder="اكتب نص الإعلان هنا للنشر اليدوي..." value={announcementText} onChange={(e) => setAnnouncementText(e.target.value)}></textarea>
                                        )}

                                        <Button className="w-full mt-2 bg-[#5c6b48] hover:bg-[#4a563a]" onClick={() => {
                                ...
                                <div className="space-y-3">
                                    {announcements.map(a => (
                                        <div key={a.id} className="bg-white p-4 rounded-xl border-r-4 border-r-[#5c6b48] shadow-sm relative border border-[#e6e2d3]">
                                            ...
                                            <p className="font-bold text-[#3d2e18] text-sm mb-1">{a.teacherName} <span className="text-[10px] text-gray-400 bg-gray-100 px-1 rounded">{a.type === 'EXAM' ? 'اختبار' : 'عام'}</span></p>
                                            {a.type === 'EXAM' && a.examDetails ? (
                                                <div className="mt-2 text-sm text-gray-700">
                                                    <p className="font-bold text-[#5c6b48] mb-1">المختبر: {a.examDetails.testerTeacherName}</p>
                                                    <ul className="space-y-1">
                                                        {a.examDetails.schedule.map((d, i) => (
                                                            <li key={i} className="bg-[#f4f1ea] p-2 rounded border border-[#e6e2d3] flex justify-between">
                                                                <span className="font-bold">{formatDateWithDay(d.date)}</span>
                                                                <span className="text-[#3d2e18]">{d.description === 'حفظ' ? 'حفظ جديد' : d.description}</span>
                                                            </li>
                                                        ))}
                                                    </ul>
                                                </div>
                                            ) : <p className="text-gray-600 text-sm">{a.content}</p>}
                                            <p className="text-[10px] text-[#8c7b60] mt-2">{formatSimpleDate(a.date)}</p>
                                        </div>
                                    ))}
                    ...
                    <div className="space-y-4 animate-slide-up">
                        <div className="flex overflow-x-auto gap-2 pb-2 mb-2 touch-pan-x bg-white p-2 rounded-xl shadow-sm border border-[#e6e2d3] no-scrollbar">
                        ...
                        {studentTab === 'LOG' && (
                            <div className="bg-white rounded-xl shadow-lg p-5 border border-[#e6e2d3] animate-fade-in">
                                <div className="flex justify-between items-center mb-4 border-b border-[#e6e2d3] pb-2">
                                    <h2 className="text-lg font-bold text-[#5c6b48] flex items-center gap-2">{currentLogId ? "تعديل سجل اليوم" : "تسجيل تسميع اليوم"}<span className="text-xs font-normal text-[#a89060] bg-[#f4f1ea] px-2 py-1 rounded-full">{logDate}</span></h2>
                                    {currentLogId && <span className="text-xs bg-amber-100 text-amber-700 px-2 py-1 rounded font-bold">وضع التعديل</span>}
                                </div>
                                <div className="mb-4 bg-[#f4f1ea] p-3 rounded-lg border border-[#e6e2d3]">
                                    <div className="flex justify-between items-center mb-2">
                                        <label className="text-xs font-bold text-[#8c7b60] block">فترات الحضور والانصراف</label>
                                        <button onClick={handleAddAttendanceSlot} className="text-xs bg-[#5c6b48] text-white px-2 py-1 rounded font-bold hover:bg-[#4a563a]">+ إضافة فترة</button>
                                    </div>
                                    {attendanceRecords.map((record, index) => (
                                        <div key={record.id} className="relative mb-3 last:mb-0 border-b last:border-0 border-gray-200 pb-3 last:pb-0">
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div><label className="text-[10px] font-bold text-[#8c7b60] mb-1 block">وقت الحضور {index + 1}</label><TimePicker value={record.arrival} onChange={(v) => handleAttendanceChange(record.id, 'arrival', v)} /></div>
                                                <div><label className="text-[10px] font-bold text-[#8c7b60] mb-1 block">وقت الانصراف {index + 1}</label><TimePicker value={record.departure || ''} onChange={(v) => handleAttendanceChange(record.id, 'departure', v)} /></div>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                <AssignmentForm title="📖 الحفظ الجديد (تسميع)" data={jadeed} onChange={(f, v) => { setJadeed({ ...jadeed, [f]: v }); markAsDirty(); }} colorClass="border-[#5c6b48] bg-[#f4f1ea]" />
                                <div className="mb-4">
                                    <div className="flex justify-between items-center mb-2">
                                        <h4 className="font-bold text-[#3d2e18] text-sm">🔄 المراجعة</h4>
                                        <button onClick={() => { setMurajaahList([...murajaahList, { ...emptyAssignment, grade: Grade.VERY_GOOD }]); markAsDirty(); }} className="text-xs bg-[#a89060] text-white px-2 py-1 rounded hover:bg-[#8f7a50] font-bold">+ إضافة</button>
                                    </div>
                                    {murajaahList.map((m, idx) => (
                                        <AssignmentForm key={idx} title={`مراجعة ${idx + 1}`} data={m} onChange={(f, v) => { const newList = [...murajaahList]; newList[idx] = { ...newList[idx], [f]: v }; setMurajaahList(newList); markAsDirty(); }} colorClass="border-[#a89060] bg-[#f4f1ea]" canRemove onRemove={() => { setMurajaahList(murajaahList.filter((_, i) => i !== idx)); markAsDirty(); }} />
                                    ))}
                                </div>
                                <div className="mb-4">
                                    <label className="text-xs font-bold text-[#8c7b60] mb-1 block">ملاحظات / رسالة لولي الأمر</label>
                                    <textarea className="w-full p-3 border border-[#e6e2d3] rounded-lg text-sm h-32 mb-2 focus:ring-2 focus:ring-[#5c6b48] outline-none" placeholder="اكتب ملاحظاتك هنا..." value={notes} onChange={(e) => { setNotes(e.target.value); markAsDirty(); }}></textarea>
                                ...
                                <Button onClick={handleSaveLog} className="w-full py-4 text-lg shadow-xl mb-4 bg-[#5c6b48] hover:bg-[#4a563a]">{currentLogId ? "💾 تحديث السجل" : "💾 حفظ السجل"}</Button>
                            </div>
                        )}
                        {studentTab === 'PLAN' && (
                        ...
                        {studentTab === 'ARCHIVE' && (
                            <div className="bg-white rounded-xl shadow-lg p-4 animate-fade-in border border-[#e6e2d3]">
                                <h3 className="font-bold text-[#3d2e18] mb-4 text-lg border-b border-[#e6e2d3] pb-2 flex items-center gap-2">🗄️ سجل التسميع السابق</h3>
                                {selectedStudent.logs.length === 0 ? (
                                    ...
                                    <div className="space-y-4">
                                        {selectedStudent.logs.map((log) => (
                                            <div key={log.id} className={`relative p-5 rounded-xl border-r-4 shadow-sm transition-all hover:shadow-md ${log.isAbsent ? 'bg-red-50 border-r-red-500' : log.isAdab ? 'bg-amber-50 border-r-amber-500' : 'bg-[#f4f1ea] border-r-[#5c6b48] border border-[#e6e2d3]'}`}>
                                                <div className="flex justify-between items-start mb-3">
                                                    <div>
                                                        <h4 className="font-bold text-[#3d2e18] flex items-center gap-2">📅 {formatSimpleDate(log.date)}</h4>
                                                        <p className="text-xs text-[#8c7b60] mt-1">{new Date(log.date).toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}</p>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        {log.isAbsent ? (<span className="bg-red-100 text-red-700 text-xs px-2 py-1 rounded font-bold">غياب ❌</span>) : log.isAdab ? (<span className="bg-amber-100 text-amber-700 text-xs px-2 py-1 rounded font-bold">يوم آداب ✨</span>) : (<span className="bg-[#5c6b48]/20 text-[#5c6b48] text-xs px-2 py-1 rounded font-bold">حضور ✅</span>)}
                                                        <ConfirmDeleteButton
                                                            label="🗑️ حذف"
                                                ...
                                                {!log.isAbsent && !log.isAdab && (
                                                    <div className="space-y-2 text-sm">
                                                        {log.jadeed && (
                                                            <div className="flex items-center gap-2 bg-white p-2 rounded border border-[#e6e2d3]">
                                                                <span className="text-[#5c6b48] font-bold">الحفظ:</span>
                                                        ...
                                                        {log.murajaah && log.murajaah.length > 0 && (
                                                            <div className="bg-white p-2 rounded border border-[#e6e2d3]">
                                                                <span className="text-[#a89060] font-bold block mb-1">المراجعة:</span>
                                                                <div className="space-y-1">
                                                ...
                                                {log.notes && (<p className="mt-2 text-xs text-gray-500 italic border-t border-[#e6e2d3] pt-2">"{log.notes.split('\n')[0]}"</p>)}
                                            </div>
                                        ))}
                                    </div>
                                )}
                        ...
                        {studentTab === 'CALC' && (<div className="bg-white rounded-xl shadow-lg p-5 animate-fade-in border border-[#e6e2d3]"><h3 className="font-bold text-[#3d2e18] mb-4 text-lg flex items-center gap-2">🧮 حاسبة إنجاز الشهر</h3><div className="space-y-4 mb-6"><div><label className="block text-xs font-bold text-[#8c7b60] mb-1">كمية الحفظ اليومية (أسطر)</label><input type="number" className="w-full p-2 border rounded-lg" placeholder="مثال: 5" value={calcAmount} onChange={e => setCalcAmount(e.target.value)} /><p className="text-[10px] text-gray-400 mt-1">* يرجى إدخال عدد الأسطر (15 سطر = 1 صفحة)</p></div><div><label className="block text-xs font-bold text-[#8c7b60] mb-1">عدد أيام الحضور في الأسبوع</label><select className="w-full p-2 border rounded-lg bg-white" value={calcWeeklyDays} onChange={e => setCalcWeeklyDays(e.target.value)}>{[1, 2, 3, 4, 5, 6, 7].map(d => (<option key={d} value={d}>{d} يوم</option>))}</select></div><div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 text-center animate-pulse">{renderCalculatorResult()}</div></div><div><label className="block text-xs font-bold text-[#8c7b60] mb-1">حفظ ملاحظات الخطة الشهرية</label><textarea className="w-full p-3 border rounded-lg h-24 focus:ring-2 focus:ring-indigo-500 outline-none text-sm" placeholder="اكتب الملاحظات أو الخطة هنا..." value={calcNotes} onChange={e => setCalcNotes(e.target.value)}></textarea><Button onClick={handleSaveCalculator} className="w-full mt-2 text-sm bg-indigo-600 hover:bg-indigo-700">حفظ الملاحظات</Button></div></div>)}
                        {studentTab === 'SCHEDULE' && (
                            <div className="bg-white rounded-xl shadow-lg p-5 animate-fade-in border border-[#e6e2d3]">
                                <h3 className="font-bold text-[#3d2e18] mb-4 text-lg">⏰ جدول الطالب والأنشطة</h3>
                                <p className="text-sm text-gray-500 mb-4 bg-[#f4f1ea] p-2 rounded">تم ملء هذا الجدول من قبل ولي الأمر.</p>
                                <div className="space-y-3">
                                    {selectedStudent.weeklySchedule.map((sched, idx) => (
                                        <div key={sched.day} className={`p-4 rounded-lg border ${sched.isDayOff ? 'bg-gray-100 border-gray-300 opacity-60' : 'bg-white border-[#e6e2d3] shadow-sm'}`}>
                                            <div className="flex justify-between items-center mb-2">
                                                <h4 className="font-bold text-[#3d2e18]">{sched.day}</h4>
                                            ...
                                            {!sched.isDayOff && (!sched.events || sched.events.length === 0) ? (
                                                ...
                                                <div className="space-y-1">
                                                    {sched.events?.map(event => (
                                                        <div key={event.id} className="flex justify-between items-center text-sm bg-[#f4f1ea] px-2 py-1 rounded border border-[#e6e2d3]">
                                                            <span className="text-[#5c6b48]">{event.title}</span>
                                                            <span className="font-mono font-bold text-[#5c6b48] bg-white px-1 rounded">{formatTime12Hour(event.time)}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
